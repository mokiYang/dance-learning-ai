...
# 镜像名称
FRONTEND_IMAGE="${REGISTRY}/${NAMESPACE}/dance-frontend:${TAG}"

echo "[信息] 标签: ${TAG}"
echo "[信息] 前端镜像: ${FRONTEND_IMAGE}"
echo
...
# 构建并推送前端镜像
echo "[信息] 构建前端 AMD64 镜像..."
cd "${SCRIPT_DIR}/frontend"

# 使用 buildx 构建 AMD64 镜像并推送
docker buildx build --platform linux/amd64 --no-cache -t "${FRONTEND_IMAGE}" --push .
